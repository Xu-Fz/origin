...
abstract public class StandardConverter extends Converter implements IConverter {
	...
	private void registerObject(Mapping mapping) throws SecurityException,
			ClassNotFoundException {
		String primaryKey = mapping.getPrimaryKey();
		String className = mapping.getClassName();
		String tableName = mapping.getTableName();
		tableName = tableName == null ? ClassUtil.getTagName(className, TOKEN)
				: tableName;
		String generator = mapping.getGenerator();
		TableMapping tm = new TableMapping(className, tableName, generator);
		java.lang.reflect.Field[] fields = Class.forName(className)
				.getDeclaredFields();
		for (java.lang.reflect.Field field : fields) {
			...
			if (column != null) {
				isIgnore = column.getIsIgnore() != null ? column.getIsIgnore()
			...
			ColumnMapping columnMapping = new ColumnMapping(propertyName,
					propertyClass, columnName, columnType, columnLimit,
					isPrimaryKey);
	...
	public String getPrimaryKeyByClassName(String className) {
		return tableMappings.get(className).getPrimaryKeyColumn()
				.getColumnName();
	}
	...
	public String getCreateTableSql(String className) {
		...
		if (createTableSql == null) {
			...
			sql.append("CREATE TABLE IF NOT EXISTS ").append(tm.getTableName())
					.append(" (").append(buffer.toString().substring(1))
					.append(");");
	...
	private static String getColumnString(boolean havePrimary,
			List<ColumnMapping> columsMappings) {
		StringBuffer buffer = new StringBuffer();
		for (ColumnMapping column : columsMappings) {
			if ((havePrimary && column.isPrimaryKey())
					|| !column.isPrimaryKey()) {
	...
	public String getLoadSql(String className) {
		...
		if (loadSql == null) {
			StringBuffer sql = new StringBuffer();
			sql.append("SELECT T.* FROM ").append(tm.getTableName())
					.append(" T where T.")
					.append(tm.getPrimaryKeyColumn().getColumnName())
					.append(" = ?;");
	...
	public String getInsertSql(String className) {
		...
		if (insertSql == null) {
			boolean isIdentity = IConverter.GENERATOR.IDENTITY
					.equalsIgnoreCase(tm.getGenerator());
			String columnString = getColumnString(!isIdentity,
					tm.getColumnMappings());
			StringBuffer sql = new StringBuffer();
			sql.append("INSERT INTO ").append(tm.getTableName()).append(" (")
					.append(columnString).append(") VALUES (");
			StringBuffer buffer = new StringBuffer();
			if (!isIdentity) {
				buffer.append(",?");
			}
	...
	public String getUpdateSql(String className) {
		...
		if (updateSql == null) {
			...
			for (ColumnMapping cm : tm.getColumnMappings()) {
				if (!cm.isPrimaryKey()) {
					buffer.append(",").append(cm.getColumnName())
							.append(" = ?");
				}
			}
	...
	public String getDeleteSql(String className) {
		...
		if (deleteSql == null) {
			StringBuffer sql = new StringBuffer();
			sql.append("DELETE FROM ").append(tm.getTableName())
					.append(" WHERE ")
					.append(tm.getPrimaryKeyColumn().getColumnName())
					.append(" = ?;");
	...
	public Object[] getInsertValues(Object instance) throws Exception {
		...
		boolean isIdentity = IConverter.GENERATOR.IDENTITY.equalsIgnoreCase(tm
				.getGenerator());
		...
		for (ColumnMapping cm : tm.getColumnMappings()) {
			if (!(cm.isPrimaryKey() && isIdentity)) {
				...
				Object value = ClassUtil.getValueByProperty(propertyName,
						instance);
	...
	public Object[] getUpdateValues(Object instance) throws Exception {
		...
		Object value = ClassUtil.getValueByProperty(primaryPropertyName,
				instance);
		objects[i] = value;
		return objects;
	}
	...
	public Serializable setIdValue(Object instance, IdBuilder idBuilder)
			throws Exception {
		TableMapping tableMapping = tableMappings.get(instance.getClass()
				.getName());
		String generator = tableMapping.getGenerator();
		if (IConverter.GENERATOR.INCREMENT.equalsIgnoreCase(generator)
				&& idBuilder != null) {
			long idValue = idBuilder.nextValue();
			ClassUtil.copyValueToProperty(instance, tableMapping
					.getPrimaryKeyColumn().getColumnName(), idValue);
			return idValue;
		}
		return null;
	}
	...
	protected String getDataLimit(boolean isPrimaryKey, String dataLimit) {
		return isPrimaryKey ? "not null" : dataLimit == null ? "null"
				: dataLimit;
	}

	protected Object readValue(Cursor cursor, Class<?> propertyClass,
			String columnName) {
		Object value = null;
		int index = cursor.getColumnIndex(columnName);
		if (long.class.equals(propertyClass)
				|| Long.class.equals(propertyClass)) {
			value = cursor.getLong(index);
		} else if (int.class.equals(propertyClass)
				|| Integer.class.equals(propertyClass)) {
			value = cursor.getInt(index);
		} else if (String.class.equals(propertyClass)) {
			value = cursor.getString(index);
		} else if (float.class.equals(propertyClass)
				|| Float.class.equals(propertyClass)) {
			value = cursor.getFloat(index);
		} else if (double.class.equals(propertyClass)
				|| Double.class.equals(propertyClass)) {
	...
	protected String getDataType(Class<?> propertyClass) {
		if (String.class.equals(propertyClass)) {
			return getColumnTypeForString();
				...
				|| int.class.equals(propertyClass)) {
